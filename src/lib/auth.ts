...
export class InvalidAuthorizationCodeError extends AuthError {}
...
export function parse_token(token: string) {
	try {
		...
		throw new InvalidAccessTokenError('Could not parse token');
	}
}
...
export async function refresh_token_request(refresh_token: string) {
	...
	if (response.status !== 200) {
		throw new InvalidRefreshTokenError('Invalid refresh_token');
	}
...
export async function refresh_tokens(cookies: Cookies) {
	...
	if (refresh_token === undefined) {
		throw new InvalidRefreshTokenError('No refresh_token');
	}
